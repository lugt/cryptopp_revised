...
# List cryptlib.cxx first, then cpu.cxx, then integer.cxx to tame C++ static initialization problems.
SRCS := cryptlib.cxx cpu.cxx integer.cxx $(filter-out cryptlib.cxx cpu.cxx integer.cxx pch.cxx simple.cxx winpipes.cxx cryptlib_bds.cxx,$(sort $(wildcard *.cxx)))
...
# List test.cxx first to tame C++ static initialization problems.
TESTSRCS := adhoc.cxx test.cxx bench1.cxx bench2.cxx bench3.cxx datatest.cxx dlltest.cxx fipsalgt.cxx validat0.cxx validat1.cxx validat2.cxx validat3.cxx validat4.cxx validat5.cxx validat6.cxx validat7.cxx validat8.cxx validat9.cxx validat10.cxx regtest1.cxx regtest2.cxx regtest3.cxx regtest4.cxx
...
dep deps depend GNUmakefile.deps:
	$(CXX) $(strip $(CXXFLAGS) -DCRYPTOPP_DISABLE_ASM) -MM *.cxx > GNUmakefile.deps
...
.PHONY: clean
clean:
	-$(RM) adhoc.cxx.o adhoc.cxx.proto.o $(LIBOBJS) rdrand-*.o $(TESTOBJS) $(DLLOBJS) $(LIBIMPORTOBJS) $(TESTIMPORTOBJS) $(DLLTESTOBJS)
...
distclean: clean autotools-clean cmake-clean
	-$(RM) adhoc.cxx adhoc.cxx.copied GNUmakefile.deps benchmarks.html cryptest.txt
...
TEXT_FILES := *.h *.cxx adhoc.cxx License.txt Readme.txt Install.txt Filelist.txt Doxyfile cryptest* cryptlib* dlltest* cryptdll* *.sln *.s *.S *.vcxproj *.filters cryptopp.rc TestVectors/*.txt TestData/*.dat TestPrograms/*.cxx TestScripts/*.sh TestScripts/*.cmd
...
trim:
ifneq ($(IS_DARWIN),0)
	sed -i '' -e's/[[:space:]]*$$//' *.supp *.txt *.sh .*.yml *.h *.cxx *.asm *.s *.S
...
adhoc.cxx: adhoc.cxx.proto
ifeq ($(wildcard adhoc.cxx),)
	cp adhoc.cxx.proto adhoc.cxx
else
	touch adhoc.cxx
endif
...
donna_32.o : donna_32.cxx
	$(CXX) $(strip $(subst -O3,-O2,$(CXXFLAGS)) -c) $<
donna_64.o : donna_64.cxx
	$(CXX) $(strip $(subst -O3,-O2,$(CXXFLAGS)) -c) $<
endif

# SSE2 on i686
sse_simd.o : sse_simd.cxx
	$(CXX) $(strip $(CXXFLAGS) $(SSE2_FLAG) -c) $<
