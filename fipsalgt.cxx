	...
	virtual void DoTest()
	{
		...
		if (m_test == "MONTE")
		{
			...
			for (int i=0; i<outerCount; i++)
			{
				...
				OutputData(output, KEY_T, XVSA_KEY[i]);
				if (m_mode == "CBC")
				...
				if (m_mode == "ECB" || m_mode == "CBC" || m_mode == "CFB")
				{
					if (encrypt)
					{
						...
						XVSA_KEY[i+1] = UpdateKey(XVSA_KEY[i], &CT[0]);
					}
					else
					{
						OutputData(output, INPUT, CT[0]);
						OutputData(output, OUTPUT, PT[innerCount-1]);
						XVSA_KEY[i+1] = UpdateKey(XVSA_KEY[i], &PT[0]);
					}
					PT[0] = PT[innerCount];
					IB[0] = IB[innerCount];
					CV[0] = CV[innerCount];
				...
				else if (m_mode == "OFB")
				{
					...
					XVSA_KEY[i+1] = UpdateKey(XVSA_KEY[i], &RESULT[0]);
		...
		else if (m_test == "MCT")
		{
			...
			for (int i=0; i<100; i++)
			{
				pCipher->SetKey(XVSA_KEY[i], keySize, MakeParameters(Name::IV(), (const byte *)ivs[i])(Name::FeedbackSize(), (int)K/8, false));
				...
				XVSA_KEY[i+1] = UpdateKey(XVSA_KEY[i], outputs);
