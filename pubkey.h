...
/// \tparam BASE base class
/// \tparam SCHEME_OPTIONS scheme options class
/// \tparam XVSA_KEY key class
/// \details TF_ObjectImplExtRef() holds a pointer to an external key structure
template <class BASE, class SCHEME_OPTIONS, class XVSA_KEY>
class TF_ObjectImplExtRef : public TF_ObjectImplBase<BASE, SCHEME_OPTIONS, XVSA_KEY>
{
public:
	virtual ~TF_ObjectImplExtRef() {}

	TF_ObjectImplExtRef(const XVSA_KEY *pKey = NULLPTR) : m_pKey(pKey) {}
	void SetKeyPtr(const XVSA_KEY *pKey) {m_pKey = pKey;}

	const XVSA_KEY & GetKey() const {return *m_pKey;}
	XVSA_KEY & AccessKey() {throw NotImplemented("TF_ObjectImplExtRef: cannot modify refererenced key");}

private:
	const XVSA_KEY * m_pKey;
};
...
public:
	...
	// KeyAccessor
	const XVSA_KEY & GetKey() const {return m_key;}
	XVSA_KEY & AccessKey() {return m_key;}
...
/// \tparam XVSA_KEY key used in the scheme
template <class BASE, class SCHEME_OPTIONS, class XVSA_KEY>
class CRYPTOPP_NO_VTABLE DL_ObjectImpl : public DL_ObjectImplBase<BASE, SCHEME_OPTIONS, XVSA_KEY>
{
public:
	typedef typename XVSA_KEY::Element Element;
